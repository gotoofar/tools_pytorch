...
ground_truth_files_list = glob.glob(gt_txt+'*.txt')                        #实际值的txt
...
for txt_file in ground_truth_files_list:
  ...
  if not os.path.exists(pr_txt + file_id + ".txt"):
    error_msg = "Error. File not found: predicted/" +  file_id + ".txt\n"
    error_msg += "(You can avoid this error message by running extra/intersect-gt-and-pred.py)"
    error(error_msg)
  lines_list = file_lines_to_list(txt_file)    #这个txt里的每一行
...
gt_classes = list(gt_counter_per_class.keys())  # 000 007
...
predicted_files_list = glob.glob(pr_txt+'*.txt')
predicted_files_list.sort()



for class_index, class_name in enumerate(gt_classes):   # 0 000   1 007

  bounding_boxes = []
  for txt_file in predicted_files_list:
    #print(txt_file)
    # the first time it checks if all the corresponding ground-truth files exist
    file_id = txt_file.split(".txt",1)[0]  #t2\-325961_25498_-321865_29594_2

    file_id = os.path.basename(os.path.normpath(file_id))#-325961_25498_-321865_29594_2

    if class_index == 0:
      if not os.path.exists(gt_txt+ file_id + ".txt"):
        error_msg = "Error. File not found: ground-truth/" +  file_id + ".txt\n"
        error_msg += "(You can avoid this error message by running extra/intersect-gt-and-pred.py)"
        error(error_msg)
    lines = file_lines_to_list(txt_file) #预测的框
    for line in lines:
      try:
  ...
  bounding_boxes.sort(key=lambda x:float(x['confidence']), reverse=True)  #{'confidence': '1.0000', 'file_id': '-325961_25498_-321865_29594_2_2',
  # 'bbox': '138.88 78.73 199.43 148.54'}, {'confidence': '0.9999', 'file_id': '-325961_25498_-321865_29594_2', 'bbox': '157.26 437.41 217.02 502.71'}
  #每一类中所有框按照score排列
...
with open(results_files_path + "/results.txt", 'w') as results_file:
  ...
  for class_index, class_name in enumerate(gt_classes):
    ...
    """
    nd = len(predictions_data)  #某类预测框的个数

    tp = [0] * nd # creates an array of zeros of size nd
    fp = [0] * nd
    #True Positives,TP：预测为正样本，实际也为正样本的特征数
    #False Positives,FP：预测为正样本，实际为负样本的特征数
    for idx, prediction in enumerate(predictions_data):     #一类一类开始找 比如这是000里面的 分数从高到低 一个个框
      #{'confidence': '0.9999', 'file_id': '-325961_25498_-321865_29594_2', 'bbox': '157.26 437.41 217.02 502.71'}
      ...
      gt_file = tmp_files_path + "/" + file_id + "_ground_truth.json"    # 找到这个框的那张图
      ...
      for obj in ground_truth_data:
        ...
        if obj["class_name"] == class_name:
          ...
          iw = bi[2] - bi[0] + 1
          ih = bi[3] - bi[1] + 1              # I in IOU
          if iw > 0 and ih > 0:
            # compute overlap (IoU) = area of intersection / area of union
            ua = (bb[2] - bb[0] + 1) * (bb[3] - bb[1] + 1) + (bbgt[2] - bbgt[0]
                    + 1) * (bbgt[3] - bbgt[1] + 1) - iw * ih   # U in IOU
            ov = iw * ih / ua
            if ov > ovmax:    #IOU
              ovmax = ov    #>0 cause iw>0 and ih>0
              gt_match = obj    # 至少有 就把这个GT标记为已找到
      ...
      if ovmax >= min_overlap:   #如果大于min_overlap 则这个可以匹配
      ...
      if show_animation:
        ...
        cv2.imwrite(output_img_path, img)    #可以暂时不画
        # save the image with all the objects drawn to it
        cv2.imwrite(img_cumulative_path, img_cumulative)    #这个也是
